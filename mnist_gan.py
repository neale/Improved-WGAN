...
def train(args):
    ...
    for iter in range(args.epochs):
        ...
        for _ in range(args.disc_iters):
            ...
            gp = ops.grad_penalty(args, sample_dist, netD, data, fake)
            gp.backward()
        ...
        if iter % 100 == 0:
            # plot.plot('results/mnist/train D cost', d_cost.cpu().item())
            print('iter: ', iter, 'train D cost', d_cost.cpu().item())
            # plot.plot('results/mnist/train G cost', g_cost.cpu().item())
            print('iter: ', iter, 'train G cost', g_cost.cpu().item())
            # plot.plot('results/mnist/w1 distance', wasserstein_d.cpu().item())
        if iter % 300 == 0:
            val_d_costs = []
            ...
            #print('val d cost', np.mean(val_d_costs))
            utils.generate_image(args, iter, netG)

        #if (total_batches < 5) or (total_batches % 100 == 99):
            #plot.flush()
        #plot.tick()
